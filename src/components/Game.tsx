...
const SHIRT_COLORS = ["#FF6B35", "#E74C3C", "#3498DB", "#2ECC71", "#9B59B6", "#FF69B4", "#F1C40F", "#1ABC9C", "#ECF0F1", "#E67E22"];

const PANTS_COLORS = ["#2C3E50", "#1a1a1a", "#1E5B2A", "#8B4513", "#7F8C8D"];
...
async function fetchLeaderboard(): Promise<LeaderboardEntry[]> {
  if (!SB_URL) return [];
  try {
    const res = await fetch(`${SB_URL}/rest/v1/leaderboard?select=id,name,time_ms,year_level&order=time_ms.asc&limit=10`, { headers: SB_HDRS });
...
function drawHUD(ctx: CanvasRenderingContext2D, score: number, level: number, levelProgress: number, levelLength: number, lives: number, playerName: string, timerMs: number) {
  // Progress bar
        ...
        <div style={{ textAlign: "center", color: "white", maxWidth: "640px", width: "100%" }}>
          ...
          >
            ...
            <div style={{ flex: "1 1 200px", textAlign: "left" }}>
              {/* Name */}
              <label style={{ color: "#ECF0F1", fontSize: "0.9rem", fontWeight: "bold", display: "block", marginBottom: "6px" }}>Your Name</label>
              <input
                type="text"
                maxLength={16}
              ...
              {/* Shirt colours */}
              <label style={{ color: "#ECF0F1", fontSize: "0.9rem", fontWeight: "bold", display: "block", marginBottom: "6px" }}>Shirt Colour</label>
              ...
              {/* Pants colours */}
              <label style={{ color: "#ECF0F1", fontSize: "0.9rem", fontWeight: "bold", display: "block", marginBottom: "6px" }}>Pants Colour</label>
    ...
    return (
      <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg, #0f3460, #533483)", overflowY: "auto", fontFamily: "Arial, sans-serif" }}>
        <div style={{ textAlign: "center", color: "white", padding: "32px 20px", maxWidth: "560px", margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: "2.2rem", margin: "0 0 6px", color: "#F39C12" }}>{skin.name ? `Amazing, ${skin.name}!` : "You Win!"}</h1>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                {leaderboard.map((entry, i) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ width: "28px", textAlign: "center", fontSize: i < 3 ? "1.1rem" : "0.85rem", color: i < 3 ? "inherit" : "#7F8C8D" }}>{i < 3 ? medals[i] : `#${i + 1}`}</span>
          ...
          <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
            <button
              onClick={() => setGameState("yearSelect")}
              style={{ padding: "14px 40px", borderRadius: "12px", border: "none", background: "#F39C12", color: "white", fontSize: "1rem", fontWeight: "bold", cursor: "pointer" }}
            >
              Play Again
            </button>
            <a
    ...
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #134e5e, #71b280)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Arial, sans-serif" }}>
        <div style={{ textAlign: "center", color: "white", padding: "40px" }}>
          ...
          <button
            onClick={nextLevel}
            style={{ padding: "16px 40px", borderRadius: "12px", border: "none", background: "#27AE60", color: "white", fontSize: "1.1rem", fontWeight: "bold", cursor: "pointer" }}
          >
            Next Level →
          </button>
        </div>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: `${CANVAS_WIDTH}px`, borderRadius: "12px", overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.6)" }}>
        ...
        {gameState === "dead" && question && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "12px" }}>
            ...
            >
              <div style={{ fontSize: "40px", marginBottom: "8px" }}>{answerResult === "correct" ? "🎉" : answerResult === "wrong" ? "😅" : "💀"}</div>
              <p style={{ color: "#7F8C8D", fontSize: "0.8rem", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "1px" }}>{question.subject}</p>
              <h2 style={{ fontSize: "1.15rem", margin: "0 0 20px", color: "#2C3E50", lineHeight: 1.4 }}>{question.question}</h2>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          ...
          <button
            onClick={() => {
              ...
              audioManager.resume();
              try { localStorage.setItem('pm_muted', String(next)); } catch { /* ignore */ }
            }}
